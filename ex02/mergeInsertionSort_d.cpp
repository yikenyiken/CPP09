...
void	insert(std::deque<iter> &dest, iterIters insertBound, iter toInsert, int len)
{
	iterIters	lastComp;
...
void	insertLosers(std::deque<iter> &dest, std::deque<iter> &winners, int pair_size)
{
	int					bound = winners.size() - 1;
	std::deque<int>		seq = js_seq_gen(bound);
	int					prev;
...
void	insertPend(std::deque<iter> &dest, std::deque<int> &vec, int n, int pair_size)
{
...
{
	std::deque<iter> winners; // stores level winners
	std::deque<iter> S; // stores sorted level elements
